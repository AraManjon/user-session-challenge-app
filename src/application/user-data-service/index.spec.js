...
describe('user data service', () => {
    ...
    it('should throw an error when is retrieved a user session with empty username', async ()=>{
        ...
        await expect(userDataService.findUser(username, token)).rejects.toThrow(Error('._a is not a valid username'));
    })
    ...
    it('should throw an error when is retrieved a user session with empty username', async ()=>{
        ...
        await expect(userDataService.findUser(username, token)).rejects.toThrow(Error('0 is not a valid token'));
    })
});
